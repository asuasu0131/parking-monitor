...
function initRods(){
  rods.forEach(r=>{
    ...
    d.style.left = (r.canvasX - colWidth/2) + "px";
    d.style.top = (r.canvasY - rowHeight/2) + "px";
    d.innerHTML = r.id;
    lot.appendChild(d);
    r.element = d;
    d.onclick = ()=>{
...
for(let r=0;r<rowCount;r++){
  for(let c=0;c<colCount;c++){
    ...
    nodes.push({row:r, col:c, x:offsetX+c*colWidth+colWidth/2, y:offsetY+r*rowHeight+rowHeight/2, neighbors:[]});
  }
}
// 隣接ノード設定
nodes.forEach(n=>{
...
function nearestRodNode(){
  ...
  // ノードに最も近いものを返す
  const goalNode = nodes.reduce((prev,curr)=> Math.hypot(curr.x-nearest.canvasX,curr.y-nearest.canvasY) < Math.hypot(prev.x-nearest.canvasX,prev.y-nearest.canvasY)?curr:prev, nodes[0]);
  return goalNode;
}
